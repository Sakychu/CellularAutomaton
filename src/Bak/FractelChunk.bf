...
namespace Plotter.Entity
{
	class FractelChunk
	{
		...
		public SDL2.Image GetRenderImage(double yMin, double yMax, double xMin, double xMax, int32 kMax, int32 xyPixelStep)
		{
			...
			/*Vector2D screenBottomLeft = scope Vector2D(xMin, yMin);
			ComplexPoint screenTopRight = scope Vector2D(xMax, yMax);
			...
			for (double y = yMin; y < yMax; y += xyStep.mY)
			{
				int xPix = 0;
				for (double x = xMin; x < xMax; x += xyStep.mX)
				{
					double cx = x;
					double cy = y;
